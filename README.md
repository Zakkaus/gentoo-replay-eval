...
- `autobump_xcheck.py` (`make xcheck`) — a second axis: cross-check the deterministic engine's decision against ground truth — does `autobump-rb --check` escalate exactly the bumps that are not offline-reproducible? Surfaced a 0% escalation-recall blind spot on metadata-driven (haskell-cabal) bumps, fixed to 92%. `round2_sample.py` re-samples a non-haskell set to confirm the signal doesn't over-fire. Full write-up in `results/AUTOBUMP_XCHECK.md`.
